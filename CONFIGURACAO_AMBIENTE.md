...
- No `.env` (ou variáveis de ambiente Docker), alinhar com as mesmas credenciais que a Evolution espera na header **`apikey`**:
  ...
  - **Imagem `evoapicloud/evolution-api` e Redis:** usar **`CACHE_REDIS_ENABLED`** + **`CACHE_REDIS_URI`** (ex.: `redis://redis:6379/0`), como no [`env.example` oficial da EvolutionAPI](https://github.com/EvolutionAPI/evolution-api/blob/main/env.example). Pares só `REDIS_ENABLED` / `REDIS_HOST` não batem com esse env; podem contribuir para instabilidade ou para **`Recv failure: Connection reset by peer`** se o Node cair a meio da resposta. O `docker-compose.yml` aqui já usa `CACHE_REDIS_*`.
  - **`POSTGRES_PASSWORD` com caracteres especiais URI (`@`, `:`, `#`, etc.):** a `DATABASE_CONNECTION_URI` pode ficar ilegível e rebentar a ligação; use percent-encoding do user/pass ou uma password só com caracteres “seguros” para URI.
